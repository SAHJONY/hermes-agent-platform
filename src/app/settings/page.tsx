...
import { User, Key, Bell, Shield, Palette, Globe, Save, Copy, Check, RefreshCw, AlertCircle } from 'lucide-react'
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div>
            ...
            <input type="text" placeholder="your@email.com" value={user?.email || ''} disabled
              className="w-full px-3 py-2.5 bg-surface border border-border rounded-lg text-sm text-white placeholder:text-zinc-500 disabled:opacity-50" />
          </div>
        </div>
    ...
    <div className="p-6">
      <div className="flex items-center justify-between mb-6">
        ...
        <Button onClick={generateKey} disabled={generating}>
          {generating ? <><RefreshCw className="h-4 w-4 animate-spin mr-2" />Generating...</> : <><Key className="h-4 w-4 mr-2" />Generate New Key</>}
        </Button>
      </div>
            ...
            <div className="flex items-start justify-between">
              <div className="flex-1">
                <div className="flex items-center gap-3 mb-2">
                  ...
                  <span className={cn('text-xs px-2 py-0.5 rounded-full', apiKey.env === 'production' ? 'bg-emerald-500/10 text-emerald-400' : 'bg-amber-500/10 text-amber-400')}>{apiKey.env}</span>
                </div>
                <div className="flex items-center gap-2">
                  <code className="text-xs text-zinc-400 bg-surface-elevated px-2 py-1 rounded font-mono">{apiKey.key}</code>
                  <button onClick={() => copyKey(apiKey.key, apiKey.id)} className="p-1 rounded hover:bg-surface-elevated transition-colors">
                    {copied === apiKey.id ? <Check className="h-4 w-4 text-emerald-400" /> : <Copy className="h-4 w-4 text-zinc-400" />}
                  </button>
                </div>
                <div className="flex items-center gap-4 mt-3 text-xs text-zinc-500"><span>Created {apiKey.created}</span><span>•</span><span>Last used {apiKey.lastUsed}</span></div>
              </div>
